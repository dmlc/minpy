...
class Solver(object):
  ...
  train_dataiter = NDArrayIter(data['X_train'],
                         ...
                         100,
                         True)
  val_dataiter = NDArrayIter(data['X_val'],
                         data['y_val'],
                         100,
                         True)

  model = MyAwesomeModel(hidden_size=100, reg=10)
  solver = Solver(model, train_dataiter, val_dataiter,
                  update_rule='sgd',
                  optim_config={
                    'learning_rate': 1e-3,
                  },
  ...
  """
    ...
    def __init__(self, model, train_dataiter, val_dataiter, **kwargs):
        """
    ...
    def check_accuracy(self, dataiter, num_samples=None, batch_size=100):
        ...
        y_pred = []
        for each_batch in dataiter:
            predict = self.model.forward(each_batch.data,
            y_pred.append(np.argmax(each_batch.label, axis=1))
