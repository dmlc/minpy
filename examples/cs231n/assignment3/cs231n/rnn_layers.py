...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for t in xrange(T):
    h[:, t, :], cache_t = rnn_step_forward(x[:, t, :], h[:, t-1, :] if t > 0 else h0, Wx, Wh, b)
...
def rnn_backward(dh, cache):
  ...
  # Load cache
  N, T, H = dh.shape
  D = cache[0][2].shape[1]  # take D from x.shape
  dx = np.zeros([N, T, D])
  dWx = np.zeros([D, H])
  dWh = np.zeros([H, H])
  db = np.zeros(H)
...
def word_embedding_backward(dout, cache):
  ...
  x, W = cache
  dW = np.zeros_like(W)
  np.add.at(dW, x, dout)  # a simple x in the second arg means indexing into 1-dim
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return dW
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = x, i, f, o, g, next_c, prev_c, Wx, Wh, prev_h
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  # load cache
  x, i, f, o, g, next_c, prev_c, Wx, Wh, prev_h = cache
  # dimentions
  N, H = f.shape
  # next_c var also contributes to dnext_h
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for t in xrange(T):
    h[:, t, :], c[:, t, :], cache_t = lstm_step_forward(
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    dnext_h_t = dprev_h_t + dh[:, t, :]
    dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h_t, dprev_c_t, cache[t])
    dx[:, t, :] = dx_t
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
