...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        ############################################################################
        l1, l1_cache = affine_relu_forward(X, self.params['W1'],
                                           self.params['b1'])
  ...
  """
    ...
    """
        ...
        for l in xrange(self.num_layers):
            ...
            self.params[self.GetWeightName(l)] = np.random.randn(
                input_d, out_d) * weight_scale
    ...
    """
        ...
        for l in xrange(self.num_layers):
            if l < (self.num_layers - 1):
                if (not self.use_batchnorm) and (not self.use_dropout):
                    last_layer_output, cache[l] = affine_relu_forward(
                        last_layer_output, self.params[self.GetWeightName(l)],
                        self.params[self.GetBiasName(l)])
                else:
                    assert False
            else:
                last_layer_output, cache[l] = affine_forward(
                    last_layer_output, self.params[self.GetWeightName(l)],
                    self.params[self.GetBiasName(l)])
        ...
        for l in xrange(self.num_layers - 1, -1, -1):
            if l == self.num_layers - 1:
                delta, grads[self.GetWeightName(l)], grads[self.GetBiasName(
                    l)] = affine_backward(delta, cache[l])
            else:
                delta, grads[self.GetWeightName(l)], grads[self.GetBiasName(
                    l)] = affine_relu_backward(delta, cache[l])

                loss += np.sum(self.params[self.GetWeightName(l)]**
                               2) * 0.5 * self.reg
                grads[self.GetWeightName(l)] += self.params[self.GetWeightName(
                    l)] * self.reg
