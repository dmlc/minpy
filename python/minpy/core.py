...
def function(symbol, input_shapes):
    ...
    Possible culprit: In model, training is complete, i.e. loss is computed in
    symbol. Not in Minpy.

        input_names = dshape.keys()
        raw_param_names = list(set(arg_names) - set(input_names))
        raw_param_names = list(set(arg_names))
        param_idx = [i for i in range(len(arg_names)) if arg_names[i] in
            raw_param_names]
    ...
    def func(*args, **kwargs):
        ...
        if len(args) > 0:
            raise MXNetSymbolArgErrorLackName(
                'find arg with no name specified')
        ...
        for name, value in kwargs.items():
            if name in executor.arg_dict:
                ...
                raise MXNetSymbolArgErrorUnknownName(
...
def ConvFunc(var, conv, basic_types):
    ...
    if type(var) in basic_types:
        ...
        conv_var = {k: ConvFunc(v, conv, basic_types)
                    for k, v in var.iteritems()}
    else:
        raise ConvertErrorUnexpectedType(
            'Unexpected %s type found in core.ConvToNumpy' % type(var))
